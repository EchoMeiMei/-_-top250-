...
#findLink=re.compile(r'<span class="title">^[\u4e00-\u9fa5]{0,}$</span>')
#影片图片的链接
...
def getData(baseurl):
    ...
    for i in range(0,25):   #调用获取页面信息的函数，10次
        ...
        for item in soup.find_all('div',class_="item"):  #查找符合要求的字符串，形成列表
            ...
            titles=re.findall(findTitle,item)     #片名可能只有一个中文名，也可能有其他名字
            if len(titles)==2:
                ctitle=titles[0]
            ...
            dataList.append(data)  #把处理好的一部电影信息追加到datalist
...
def askURL(url):
    ...
    # 用户代理，表示告诉服务器，我们是什么样的信息（本质上是告诉浏览器，我们可以接受什么水平的信息）
